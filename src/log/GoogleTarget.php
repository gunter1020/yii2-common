    ...
    protected function getGoogleLogEntry($message)
    {
        ...
        return [
            ...
            'httpRequest' => [
                ...
                'requestSize' => StringHelper::byteLength($request->getRawBody()) . 'bytes',
                'status' => $response->getStatusCode(),
                'responseSize' => StringHelper::byteLength($response->content) . 'bytes',
                'userAgent' => $request->getUserAgent(),
                'remoteIp' => $request->getUserIP(),
                'serverIp' => $_SERVER['SERVER_ADDR'],
                'referer' => $_SERVER['HTTP_REFERER'],
                'latency' => round(YII_BEGIN_TIME - $_SERVER['REQUEST_TIME_FLOAT'], 4) . 's',
